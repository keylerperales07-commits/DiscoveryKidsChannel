...
### 🔧 NextProgram2: 30 segundos exactos

`NEXTPROGRAM_SHOW_BEFORE_MS` (único, 30,7s, usado para los dos GIFs) se partió en dos constantes — `NEXTPROGRAM1_SHOW_BEFORE_MS` (30,7s, sin cambios) y `NEXTPROGRAM2_SHOW_BEFORE_MS` (30s exactos, como se pidió) — reusando la misma detección de GIF activo que ya usa el recuadro (`isCurrentNextProgramFactory2()`, extraída a función propia para reusarla en `scheduleNextProgramBug()`).

### 🆕 Agregado

- **Nueva app: Discovery Kids HD.** A partir de esta RELEASE, el asset del release en GitHub incluye, además del `.apk` de siempre, un **nuevo `.apk` de la variante HD** — el proyecto hermano sin CRT, sin música de fondo, sin el forzado 4:3, pensado para reproducir contenido en alta definición. Ver la sección [📺 Discovery Kids HD](#-discovery-kids-hd) más abajo.

### 🔧 Corregido

- **Crash al iniciar cuando cambia el horario, e ícono de la app que no cambiaba — la misma causa.** La versión anterior llamaba a `PackageManager.setComponentEnabledSetting()` en **todas** las aperturas de la app, sin importar si hacía falta. Llamarlo justo en el momento en que cambia la franja horaria significa deshabilitar el mismo `activity-alias` que el sistema acaba de usar para abrir la app — lo cual puede hacer que el sistema mate la tarea en pleno arranque (el crash de "una sola vez, y al reabrir ya está normal"). Como el toggle nunca llegaba a completarse limpio en ese momento, el ícono quedaba a mitad de camino — de ahí que pareciera que "no cambia". Se corrigió con dos cambios: (1) solo se llama a `setComponentEnabledSetting()` si la franja realmente cambió desde la última vez (se guarda en `SharedPreferences`), y (2) todo el bloque va en `try/catch`, por si el sistema igual llegara a rechazar el cambio en el peor momento — ya no vuelve a crashear la app.

### 🎨 Cambiado — Transparencia y colores del Launcher

- El panel de contenido del Launcher (antes blanco/crema opaco) pasa a ser **semitransparente** (~85%), para que el fondo de horario se siga viendo detrás.
- Los "cuadros dentro de otro cuadro" que no tenían un color propio asignado (la tarjeta "Cantidad de programas", cada fila de programa) ahora usan un **amarillento transparente**, el mismo tono que los botones.
- El texto del Launcher pasa a **negro**; los títulos (secciones, nombre de cada programa) pasan a **amarillo**, como los botones.

### 🗑️ Eliminado — Funciones Experimentales

Se elimina por completo el interruptor maestro "Habilitar funciones experimentales" de Configuración (con su diálogo de "reiniciar ahora o más tarde"). Discovery Kids Launcher deja de estar detrás de ningún interruptor — es simplemente la pantalla de inicio de la app, siempre. Todo lo que antes solo funcionaba con Experimental activado (cantidad de programas configurable, video elegido por el usuario para cada programa, continuamos personalizado, Intro/Créditos) ahora aplica siempre.

### 🔧 Notas técnicas

- `LiveDiscoveryKids.kt`: `totalProgramCount()`, `hasValidIntro()`, `hasValidCreditos()`, `resolveProgram()`, `resolveContinuamosUri()` ya no chequean `isExperimentalEnabled()` (eliminada). Nueva `isCurrentNextProgramFactory2()` (extraída de `showVideoInBox()`), usada también por `scheduleNextProgramBug()`.
- `SettingsManager.kt`: eliminados `KEY_EXPERIMENTAL_ENABLED`, `DEFAULT_EXPERIMENTAL_ENABLED`, `isExperimentalEnabled()`, `setExperimentalEnabled()`. Nuevos `KEY_LAST_TIME_OF_DAY_ICON`, `getLastTimeOfDayIcon()`, `setLastTimeOfDayIcon()`.
- `SettingsActivity.kt` / `activity_settings.xml`: eliminada la sección "Experimental" completa (switch, listener, `showRestartDialog()`).
- `DiscoveryKidsLauncherActivity.kt`: `onCreate()`/`onResume()` ya no redirigen según Experimental — siempre muestran la UI. `applyTimeOfDayIcon()` reescrita con guardia de "¿cambió la franja?" + `try/catch`.
- `AndroidManifest.xml`: `<application>` y los dos `<activity-alias>` pasan de íconos/label de Preview (`ic_launcher_preview`, `beta_app_name`) a los de RELEASE (`ic_launcher`, `release_app_name`) — mañana usa `ic_launcher`/`ic_launcher_round`, tarde/noche usa `ic_launcher_tarde`/`ic_launcher_round_tarde`.
- Nuevos drawables: `bg_launcher_card_yellow.xml`. `bg_launcher_panel.xml` pasa a usar `dk_launcher_panel_bg_transparent`. Nuevos colores en `colors.xml` (`dk_launcher_panel_bg_transparent`, `dk_launcher_card_yellow`, `dk_launcher_text_black`, `dk_launcher_title_yellow`).
- `build.gradle.kts`: `versionName` a `2013.6.0.0.01`.

### ⚠️ Alcance

> Cambios de código en `LiveDiscoveryKids.kt`, `SettingsManager.kt`, `SettingsActivity.kt` / `activity_settings.xml`, `DiscoveryKidsLauncherActivity.kt` / `activity_launcher.xml` / `item_program_row.xml`, `ProgramConfigActivity.kt` / `item_program_config.xml` (limpieza de comentarios), `AndroidManifest.xml`, `colors.xml`, nuevos drawables. `build.gradle.kts`: `versionName` a `2013.6.0.0.01`.
>
> ⚠️ Sigue haciendo falta agregar aparte: los íconos de tarde/noche (`ic_launcher_tarde`, `ic_launcher_round_tarde`, y sus variantes `_preview` para la rama Preview) — obligatorio para compilar. Los fondos ilustrados reales del Launcher siguen siendo opcionales (hoy son degradados placeholder).
...
### ⚙️ Reposicionamiento del Bumper dentro del corte comercial

El Bumper ya no es el primer ítem del ciclo (ya no sale antes del programa). Ahora se inserta de forma aleatoria dentro de cada corte comercial, en uno de dos puntos posibles — sorteado una sola vez por corte:
...
Nuevo set de Bumpers correspondiente a la UPDATE "La Era Planetaria".

### 🔧 Notas técnicas (`LiveDiscoveryKids.kt`)

- `buildPlaylist()` ya no agrega `PlayItem.Bumper` al ciclo del playlist (el ciclo queda `[Intro] → Programa → [Créditos]`). El tipo `PlayItem.Bumper` y su rama en `advance()` se dejan intactos, sin uso, por si se necesitan más adelante.
- Nuevo campo de instancia `commercialBumperBeforeComercial` (booleano, sorteado en `playCommercial()` con `listOf(true, false).random()`) decide, para cada corte comercial, en cuál de los dos puntos posibles sale el Bumper de ese corte.
- Nuevos valores `BUMPER_BEFORE_COMERCIAL` y `BUMPER_AFTER_COMERCIAL` en el enum `CommercialStep`, para que `resumeCommercialBlock()` pueda reconstruir el Bumper exacto (mismo recurso, misma posición) si la app pasa a segundo plano justo mientras suena. Nuevo campo `commercialChosenBumperRes` guarda ese recurso, igual que ya se hacía con `commercialChosenCommercial` / `commercialChosenPreComercial` / `commercialChosenYaVolvemos`.
- `playCommercialStepPreComercial()` se dividió en tres funciones encadenadas para poder insertar el Bumper en cualquiera de los dos puntos sin duplicar lógica: `playCommercialStepPreComercial()` (paso 1, sin cambios de comportamiento salvo el gancho al Bumper), `playCommercialStepComercial()` (paso 2, nueva) y `playCommercialStepPostComercial()` (paso 3, nueva).
- Nueva función `playCommercialBumper()`: reproduce un Bumper aleatorio (sin repetir el último, igual que el Bumper clásico) dentro del corte comercial, encadenando al siguiente paso vía callback en vez de llamar `advance()`.

### ⚠️ Alcance

> Cambios de código en `LiveDiscoveryKids.kt` (`buildPlaylist()`, `playCommercial()`, `CommercialStep`, `resumeCommercialBlock()`, división de `playCommercialStepPreComercial()` en tres pasos, nueva `playCommercialBumper()`). Contenido: nuevos Bumpers y 2 comerciales actualizados (UPDATE "La Era Planetaria"). `build.gradle`: `versionName` a `2013.6.0.0.1`.
