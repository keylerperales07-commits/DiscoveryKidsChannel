...
class LiveDiscoveryKids : AppCompatActivity() {
    ...
    private lateinit var settingsButton: ImageButton  // Preview 2006.4.1.0.11
    ...
    private var bugShowDelayMs: Long = SettingsManager.DEFAULT_SCREENBUG_DELAY_SEC * 1_000L
    private var breakIntervalMinMs: Long = SettingsManager.DEFAULT_COMMERCIAL_MIN_MINUTES * 60 * 1_000L
    private var breakIntervalMaxMs: Long = SettingsManager.DEFAULT_COMMERCIAL_MAX_MINUTES * 60 * 1_000L
    ...
    // a llamar advance().
    private var currentClipUri: Uri?            = null
    private var currentClipPositionMs           = 0
    private var currentClipOnComplete: (() -> Unit)? = null
    ...
    private enum class CommercialStep { PRE_COMERCIAL, COMERCIAL, POST_COMERCIAL }
    private var commercialStep: CommercialStep        = CommercialStep.PRE_COMERCIAL
    private var commercialChosenPreComercial: Int      = -1
    private var commercialChosenCommercial: Int        = -1
    private var commercialChosenYaVolvemos: Int         = -1
    ...
    companion object {
        ...
        private const val BREAK_CUTOFF_MS = 3 * 60 * 1_000L         // 3 min
    ...
    */
    private fun beginProgramSegment(
        uri: Uri,
        startOffsetMs: Int,
        isFirstPlay: Boolean,
     ...
     */
    private fun scheduleSegmentLogic(segmentStartMs: Int, isNewSegment: Boolean) {
        ...
        if (elapsed >= bugShowDelayMs) {
            ...
            Log.d(TAG, "ScreenBug: elapsed=${elapsed}ms >= bugShowDelayMs(${bugShowDelayMs}) → aparece inmediatamente")
            setBugAlpha(1f)
    ...
    private fun playCommercial(resumeProgramAtMs: Int) {
        ...
        val chosenPreComercial = ENSEGUIDAS_PRE_COMERCIAL[currentProgramIndex % ENSEGUIDAS_PRE_COMERCIAL.size]
        ...
        commercialStep = CommercialStep.PRE_COMERCIAL
    ...
    private fun playCommercialStepPreComercial(
     ...
     */
    private fun resumeCommercialBlock(startOffsetMs: Int) {
        when (commercialStep) {
            CommercialStep.PRE_COMERCIAL -> {
                Log.d(TAG, "onResume – reanudando ya_regresa (pre-comercial) en ${startOffsetMs}ms")
            ...
            CommercialStep.COMERCIAL -> {
                Log.d(TAG, "onResume – reanudando comercial en ${startOffsetMs}ms")
                resumeUriWithSeek(rawUri(commercialChosenCommercial), startOffsetMs) {
                    Log.d(TAG, "▶ YA VOLVEMOS post-comercial [res=$commercialChosenYaVolvemos]")
                    commercialStep = CommercialStep.POST_COMERCIAL
            ...
            CommercialStep.POST_COMERCIAL -> {
                Log.d(TAG, "onResume – reanudando continuamos en ${startOffsetMs}ms")
    ...
    private fun calcBreaks(durationMs: Int): List<Int> {
        ...
        val cutoff = durationMs - BREAK_CUTOFF_MS
    ...
    private fun startBgMusic() {
        if (!SettingsManager.isBgMusicEnabled(this)) {  // Preview 2006.4.1.0.11
            Log.d(TAG, "BG Music SKIPPED – deshabilitada en Configuración")
            return
        }
        if (bgPlayer == null) {
    ...
    private fun stopPositionTracker() {
        positionTrackerHandler.removeCallbacksAndMessages(null)
        val loggedPos = if (isInProgramSegment) pausedPositionMs else currentClipPositionMs
        Log.d(TAG, "PositionTracker STOPPED at ${loggedPos}ms (tipo=$currentItemType)")
    }
