...
/**
 * AppUpdater — Preview 2006.4.2.0.20
 *
 * Actualizador integrado: consulta el último release publicado en GitHub
 * (`keylerperales07-commits/DiscoveryKidsChannel`), compara su versión contra
 * la instalada (`versionName`, vía PackageManager) y, si hay una más nueva,
 * descarga el `.apk` adjunto al release usando `DownloadManager` y abre el
 * instalador del sistema al terminar.
 *
 * Accesible únicamente desde Configuración → "Buscar actualizaciones"
 * (sección nueva "Actualizaciones"). No corre automáticamente al iniciar la
 * app: siempre es una acción explícita del usuario.
 *
 * Flujo:
 *   1. checkForUpdate() — hilo en background, GET a la API de GitHub Releases
 *      (`/releases/latest`), parsea `tag_name` y la URL del primer asset
 *      `.apk` en `assets[].browser_download_url`.
 *   2. Compara `tag_name` (sin el prefijo "v" si lo tuviera) contra
 ...
 *   3. Si hay versión nueva: AlertDialog de confirmación → downloadAndInstall()
 ...
 *   4. Si no hay versión nueva, o si falla la consulta (sin red, repo sin
...
object AppUpdater {
    ...
    // ── Paso 1: consultar el último release ─────────────────────────────────
    fun checkForUpdate(context: Context, callback: CheckCallback) {
        Thread {
            try {
                val json = httpGet(GITHUB_API_LATEST_RELEASE)
