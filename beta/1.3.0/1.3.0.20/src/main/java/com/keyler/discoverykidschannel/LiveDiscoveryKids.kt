 ...
 * Bumper (bumper.mp4) and commercials (comercial1.mp4, comercial2.mp4) are bundled in res/raw.
...
class LiveDiscoveryKids : AppCompatActivity() {
    ...
    companion object {
        ...
        private val COMMERCIALS = listOf(R.raw.comercial1, R.raw.comercial2, R.raw.comercial3)
    }
    ...
    */
    ...
    private fun playCommercial(resumeProgramAtMs: Int) {
        ...
        // Elige un comercial al azar AQUÍ, en cada llamada a playCommercial,
        // evitando repetir el mismo que se reprodujo la vez anterior.
        val candidates = COMMERCIALS.filter { it != lastCommercialRes }
            .ifEmpty { COMMERCIALS }          // fallback si solo hay 1 comercial
        val chosenCommercial = candidates.random()
        lastCommercialRes = chosenCommercial

        // Secuencia: ya_volvemos → comercial (aleatorio) → ya_volvemos → programa
        playUri(rawUri(R.raw.ya_volvemos)) {
            Log.d(TAG, "▶ COMMERCIAL ${chosenCommercial} (resumes program at ${resumeProgramAtMs}ms)")
     ...
     * Si ya hay un MediaPlayer activo y reproduciéndose, no hace nada
     * (evita reiniciar la canción al volver de un comercial).
     * Si el player existe pero estaba pausado, lo reanuda desde donde quedó.
     */
    private fun startBgMusic() {
        if (bgPlayer == null) {
            // Primera vez: crear y configurar el MediaPlayer
            bgPlayer = MediaPlayer.create(this, R.raw.bg_music)?.apply {
                isLooping = true   // se repite indefinidamente
                setVolume(0.4f, 0.4f)  // volumen al 40% para no tapar el audio del video
                start()
