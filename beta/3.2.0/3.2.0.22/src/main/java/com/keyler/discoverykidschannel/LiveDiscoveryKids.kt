 ...
 * Beta 2003.3.2.0.22
 *
 * Cambios respecto a 3.2.0.21:
 *   - Distribución de cortes comerciales cambiada a intervalo fijo de 9 minutos.
 *     calcBreaks() ahora coloca breaks exactamente en [9 min, 18 min, 27 min, ...]
 *     en lugar de distribuirlos de forma equidistante. El primer corte ocurre
 *     siempre a los 9 minutos, y cada corte subsiguiente cada 9 minutos adicionales.
 *   - FadeOut diferenciado por tipo de clip: enseguida = 1 000 ms, bumper = 700 ms.
 *     playUriWithTransition() acepta un parámetro fadeOutMs (default TRANSITION_FADE_OUT_MS = 2 s)
 *     que permite ajustar el FadeOut de salida por tipo de clip sin alterar la constante global.
 *     Constantes nuevas: ENSEGUIDA_FADE_OUT_MS = 1_000L, BUMPER_FADE_OUT_MS = 700L.
 *
 *
 *
...
class LiveDiscoveryKids : AppCompatActivity() {
    ...
    companion object {
        ...
        /** FadeOut duration for video transitions (ms). Applied before every video change (default). */
        ...
        /** Beta 3.2.0.22 — FadeOut diferenciado: enseguida 1 s, bumper 700 ms, ya_regresa 500 ms, continuamos 500 ms. */
        private const val ENSEGUIDA_FADE_OUT_MS  = 500L   // FadeOut de salida para enseguidas
        private const val BUMPER_FADE_OUT_MS     = 500L     // FadeOut de salida para bumpers
        private const val YA_REGRESA_FADE_OUT_MS = 500L     // FadeOut de salida para ya_regresa
        private const val CONTINUAMOS_FADE_OUT_MS = 500L    // FadeOut de salida para continuamos
    ...
    private fun playBumper() {
        ...
        playUriWithTransition(rawUri(chosenBumper), fadeOutMs = BUMPER_FADE_OUT_MS) {
            playlistIndex++
            advance()
        }
    }
    ...
    private fun playEnseguida() {
        ...
        playUriWithTransition(rawUri(chosenEnseguida), fadeOutMs = ENSEGUIDA_FADE_OUT_MS) {
            playlistIndex++
            advance()
        }
    }
    ...
    */
        ...
        videoView.animate()
            ...
            .withEndAction {
                ...
                videoView.setOnCompletionListener {
                    ...
                    // Paso 2: comercial — el FadeOut de salida del ya_regresa es 500 ms
                    playUriWithTransition(rawUri(chosenCommercial), fadeOutMs = YA_REGRESA_FADE_OUT_MS) {
                        Log.d(TAG, "▶ YA VOLVEMOS post-comercial [res=$chosenYaVolvemos]")

                        // Paso 3: continuamos (FadeOut salida 500 ms / FadeIn 1 s)
                        playUriWithTransition(rawUri(chosenYaVolvemos), fadeOutMs = CONTINUAMOS_FADE_OUT_MS) {
     ...
     *      [transitionCompleted]).
     *
     * [fadeOutMs] controla la duración del FadeOut de SALIDA del clip actual
     * (default: TRANSITION_FADE_OUT_MS = 2 s). Callers pueden pasar valores
     * distintos — ENSEGUIDA_FADE_OUT_MS (1 s), BUMPER_FADE_OUT_MS (700 ms),
     * YA_REGRESA_FADE_OUT_MS (500 ms) o CONTINUAMOS_FADE_OUT_MS (500 ms) —
     * para ajustar el ritmo visual por tipo de clip sin alterar la constante global.
    ...
    private fun playUriWithTransition(
        ...
        // FadeOut de cierre del clip anterior con duración configurable por tipo de clip
        videoView.animate()
            .alpha(0f)
