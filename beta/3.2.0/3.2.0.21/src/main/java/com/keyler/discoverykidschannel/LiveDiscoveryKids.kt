 ...
 * Beta 2003.3.2.0.21
 *
 * Cambios respecto a 3.2.0.20:
 *   - BUG FIX: el FadeOut de transición ahora se dispara 2 segundos ANTES del fin del video
 *     en lugar de esperar a que el video haya completado. Implementado mediante
 *     scheduleTransitionFadeOut() que posta el FadeOut a (duration - TRANSITION_FADE_OUT_MS).
 *     onCompletionListener queda solo como fallback para videos muy cortos.
 *   - Sistema de ya_regresa por programa: selección mediante "shuffled pool" de 4 elementos.
 *     En cada ciclo de 4 programas se asigna un ya_regresa distinto a cada uno, sin repetición
 *     dentro del ciclo. Al agotarse el pool (cada 4 programas) se regenera con nuevo shuffle.
 *     Elimina la selección simple de anti-repetición que aún podía repetir en ciclos de 2.
 *
 * Beta 2003.3.2.0.20
 *   - Animaciones de transición profesionales en TODOS los cambios de video.
 *   - FadeOut de 2000 ms antes de cada corte; FadeIn de 1000 ms al iniciar el nuevo clip.
 *   - Se aplica a: fin de programa → enseguida, enseguida, bumper, StandaloneCommercial,
 *     ya_regresa (pre-comercial), comercial, continuamos (post-comercial) y retoma de programa.
 *   - Nuevo helper playUriWithTransition() centraliza la lógica de transición para
 *     garantizar consistencia en toda la secuencia de canal.
 ...
 *
 * Playlist sequence (all transitions: FadeOut 2s / FadeIn 1s):
...
class LiveDiscoveryKids : AppCompatActivity() {
    ...
    // ── ya_regresa shuffled pool (Beta 3.2.0.21) ──────────────────────────────
    // Cada ciclo de 4 programas recibe un ya_regresa distinto, sin repetición.
    // yaRegresaPool contiene los 4 recursos mezclados para el ciclo en curso.
    // yaRegresaPoolIndex apunta al siguiente slot a usar; al llegar a 4 se regenera.
    private var yaRegresaPool: MutableList<Int> = mutableListOf()
    private var yaRegresaPoolIndex: Int = 4   // empieza en 4 para forzar generación en el primer uso
    // Conservado por compatibilidad con referencias internas de saveChannelState
    ...
    companion object {
        ...
        /** FadeOut duration for video transitions (ms). Applied before every video change. */
        private const val TRANSITION_FADE_OUT_MS = 2_000L
    ...
    private fun playBumper() {
        ...
        // Beta 2003.3.2.0.20: transición con FadeOut 2s / FadeIn 1s
    ...
    */
    private fun beginProgramSegment(uri: Uri, startOffsetMs: Int, isFirstPlay: Boolean) {
        ...
        videoView.setOnPreparedListener { mp ->
            ...
            // Beta 2003.3.2.0.20: FadeIn del programa usa TRANSITION_FADE_IN_MS (1 s),
            // consistente con el FadeIn de todos los demás clips del canal.
            // En versiones anteriores era 500 ms fijo.
            videoView.alpha = 0f
            videoView.start()
            videoView.animate()
                .alpha(1f)
    ...
    private fun playCommercial(resumeProgramAtMs: Int) {
        ...
        // Beta 3.2.0.21 — ya_regresa por shuffled pool.
        // Cada ciclo de 4 programas recibe un ya_regresa distinto (sin repetición).
        // Al agotarse los 4 slots del ciclo se regenera el pool con un nuevo shuffle.
        ...
        // Beta 2003.3.2.0.20: FadeOut de 2 s antes de iniciar la secuencia comercial.
        // Reemplaza el FadeOut de 500 ms de la versión 2.4.1.
        // Cada paso interno (ya_regresa → comercial → continuamos) usa también
        // playUriWithTransition para mantener la animación 2s/1s en toda la cadena.
        //
        // BUG FIX 2001.2.5.0.52 se conserva: videoView.alpha = 1f ya no es necesario
        // como corrección explícita porque playUriWithTransition siempre establece
        // alpha = 0f antes del FadeIn, garantizando visibilidad desde el primer frame.
        videoView.animate()
            .alpha(0f)
            .setDuration(TRANSITION_FADE_OUT_MS)
            .withEndAction {
                // Paso 1: ya_regresa (pre-comercial) con FadeIn 1s
                videoView.alpha = 0f
                videoView.setOnPreparedListener { mp ->
                    mp.isLooping = false
                    videoView.start()
                ...
                videoView.setOnCompletionListener {
                    ...
                    // Paso 2: comercial con FadeOut 2s / FadeIn 1s
                    playUriWithTransition(rawUri(chosenCommercial)) {
                        Log.d(TAG, "▶ YA VOLVEMOS post-comercial [res=$chosenYaVolvemos]")

                        // Paso 3: continuamos con FadeOut 2s / FadeIn 1s
                        playUriWithTransition(rawUri(chosenYaVolvemos)) {
                            // Paso 4: retomar programa (beginProgramSegment maneja su propio FadeIn)
     ...
     *      max(0, duration - TRANSITION_FADE_OUT_MS). Esto dispara la animación
     *      2 segundos antes del fin real del clip.
     ...
     *      que TRANSITION_FADE_OUT_MS (el handler ya habrá ejecutado onComplete
     ...
     */
    private fun playUriWithTransition(uri: Uri, onComplete: () -> Unit) {
        // FadeOut de cierre del clip anterior (pantalla → negro antes de cambiar URI)
        videoView.animate()
            .alpha(0f)
            .setDuration(TRANSITION_FADE_OUT_MS)
            .withEndAction {
                ...
                videoView.setOnPreparedListener { mp ->
                    mp.isLooping = false
                    // FadeIn: 1 segundo para abrir el nuevo clip
                    videoView.alpha = 0f
                    videoView.start()
                    videoView.animate()
                        .alpha(1f)
                    ...
                    // Beta 3.2.0.21: schedular FadeOut 2 s ANTES del fin del video
                    val duration = mp.duration
                    val fadeOutDelay = (duration - TRANSITION_FADE_OUT_MS).coerceAtLeast(0L)
                    post(fadeOutDelay) {
                        if (!transitionCompleted) {
                            transitionCompleted = true
                            // Cancelar el onCompletionListener para que no dispare doble
                            videoView.setOnCompletionListener(null)
                            // Iniciar FadeOut y al terminar ejecutar onComplete
                            videoView.animate()
                                .alpha(0f)
                                .setDuration(TRANSITION_FADE_OUT_MS)
                ...
                // Fallback: si el clip es más corto que TRANSITION_FADE_OUT_MS
                // o el handler llega tarde, onCompletion asegura que onComplete
                // se ejecute de todas formas.
                videoView.setOnCompletionListener {
                    if (!transitionCompleted) {
                        transitionCompleted = true
                        onComplete()
