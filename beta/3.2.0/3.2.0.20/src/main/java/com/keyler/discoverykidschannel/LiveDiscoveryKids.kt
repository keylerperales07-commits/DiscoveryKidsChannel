...
class LiveDiscoveryKids : AppCompatActivity() {
    ...
    */
    ...
    /**
     * Beta 2003.3.2.0.20 — Transición profesional entre clips.
     *
     * Ejecuta un FadeOut de [TRANSITION_FADE_OUT_MS] (2 s) sobre el VideoView,
     * luego inicia [uri] y aplica un FadeIn de [TRANSITION_FADE_IN_MS] (1 s)
     * al arrancar la reproducción. Al completarse el clip llama a [onComplete].
     *
    ...
    private fun playUriWithTransition(uri: Uri, onComplete: () -> Unit) {
        // FadeOut: 2 segundos para cerrar el clip anterior
        videoView.animate()
            .alpha(0f)
            .setDuration(TRANSITION_FADE_OUT_MS)
            .withEndAction {
                // Configurar el nuevo clip una vez que la pantalla está en negro
                videoView.setOnPreparedListener { mp ->
                    mp.isLooping = false
