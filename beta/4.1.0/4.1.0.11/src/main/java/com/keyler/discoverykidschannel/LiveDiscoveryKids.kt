 ...
 *
 * Configuración (Preview 4.1.0.11): SettingsActivity, accesible desde el botón ⚙️,
 * permite alternar música de fondo, modo debug y brillo del CRT. Ver SettingsManager.
 */
class LiveDiscoveryKids : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        applySettings()  // Preview 2006.4.1.0.11: lee SettingsManager antes de mostrar nada
        setupDebugInfo()
        displayInfo()
    ...
    override fun onResume() {
        ...
        // Preview 2006.4.1.0.11: refresca brillo CRT / debug por si el usuario
        // volvió de SettingsActivity con cambios. Se hace siempre, sin importar
        // pausedByLifecycle, porque abrir SettingsActivity también pasa por
        // onPause()/onResume() aunque no sea un backgrounding real del sistema.
        applySettings()
    ...
    */
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // Configuración – Preview 2006.4.1.0.11
    // ══════════════════════════════════════════════════════════════════════════
     ...
     * en SettingsActivity y volvió). El brillo del CRT se aplica siempre aquí
     * porque crtOverlay no tiene su propio mecanismo de refresco automático;
     * música y debug se resuelven solos en su próximo ciclo (startBgMusic /
     * setupDebugInfo ya consultan SettingsManager directamente).
     */
    private fun applySettings() {
        crtOverlay.brightnessMultiplier = SettingsManager.getCrtBrightness(this)
    }

    /**
     * Re-evalúa el modo debug al volver de SettingsActivity. Limpia el
     * debugHandler antes de llamar setupDebugInfo() para evitar que se
     * acumulen múltiples updateTask corriendo en paralelo si el usuario
     * entra y sale de Configuración varias veces con el debug activado.
     */
    private fun refreshDebugVisibility() {
    ...
    private fun setupDebugInfo() {
        ...
        // Preview 2006.4.1.0.11: si el modo debug está deshabilitado en
        // Configuración, se oculta el overlay y no se arranca el monitor de RAM/FPS.
        if (!SettingsManager.isDebugModeEnabled(this)) {
