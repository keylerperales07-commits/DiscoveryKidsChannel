 ...
 * Release 2006.4.1.0.11-preview – Discovery Kids Channel · Era Doki 1.0 (2005–2009)
 *
 ...
 *
 * BRILLO CONFIGURABLE (2006.4.1.0.11-preview)
 *     brightnessMultiplier (0.0–1.0) escala todos los alphas anteriores sin
 *     modificar los valores base de la Era. Se controla desde SettingsActivity
 *     ("Brillo del CRT") vía SettingsManager y se aplica en LiveDiscoveryKids.onCreate().
 */
class CrtOverlayView @JvmOverloads constructor(
    ...
    /**
     * Multiplicador global de intensidad (0.0 = overlay invisible, 1.0 = intensidad base
     * definida por los parámetros de arriba). Preview 2006.4.1.0.11: controlado desde
     * SettingsActivity ("Brillo del CRT"). Se aplica a TODOS los alphas en onDraw,
     * nunca modifica los valores base, así que cambiar el brillo no afecta la
     * identidad visual de la Era una vez vuelto a 1.0.
     */
    var brightnessMultiplier: Float = 1f
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val globalAlpha = (1f - flickerIntensity * brightnessMultiplier * flickerVal).coerceIn(0f, 1f)
        alpha = globalAlpha
        ...
        scanlinePaint.alpha = (scanlineAlpha * brightnessMultiplier).toInt()
        scanlineGlowPaint.alpha = (scanlineGlowAlpha * brightnessMultiplier).toInt()
        ...
        val scaledPhosphorAlpha = (phosphorAlpha * brightnessMultiplier).toInt()
        ...
        // ── Vignette + Border shaders (se reconstruyen si cambia el tamaño o el brillo) ──
        if (w.toInt() != cachedWidth || h.toInt() != cachedHeight || brightnessMultiplier != cachedBrightness) {
            cachedWidth  = w.toInt()
            cachedHeight = h.toInt()
            cachedBrightness = brightnessMultiplier
            val scaledVignetteAlpha = (vignetteAlpha * brightnessMultiplier).toInt()
            val cx = w / 2f
            val cy = h / 2f
            ...
            val blackEdge  = Color.argb((borderAlpha * brightnessMultiplier).toInt(), 0, 0, 0)
