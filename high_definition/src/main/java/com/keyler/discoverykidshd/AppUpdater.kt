 ...
 *
 * RELEASE 2013.6.0.0-HD — CAMBIO: la selección del asset .apk dentro de
 * cada release ahora filtra por nombre (debe contener "-HD" antes de
 * ".apk", ej. "6.0.0-release-HD.apk") en vez de tomar el primer .apk
 * adjunto sin más. Ver checkForUpdate() para el detalle.
 */
object AppUpdater {
    ...
    fun checkForUpdate(context: Context, callback: CheckCallback) {
        Thread {
            try {
                ...
                // RELEASE 2013.6.0.0-HD — antes se tomaba el primer asset que
                // terminara en ".apk" sin más filtro. Ahora cada release de
                // GitHub puede traer más de un .apk adjunto (ej. la versión
                // "normal" del canal y esta variante HD del mismo repo), así
                // que hace falta filtrar por nombre: solo se acepta el asset
                // cuyo nombre contenga "-HD" antes de la extensión (ej.
                // "6.0.0-release-HD.apk", "6.0.0-preview-HD.apk"). Si el
                // release no trae ningún asset con ese patrón, apkUrl queda
                // en null y más abajo se informa el mismo error que antes
                // ("no tiene un .apk adjunto"), aunque el release sí tenga
                // otros .apk que no sean de esta variante.
                ...
                if (assets != null) {
                    for (i in 0 until assets.length()) {
                        ...
                        if (name.endsWith(".apk", ignoreCase = true) &&
                            name.contains("-HD", ignoreCase = true)
                        ) {
    ...
    private const val DOWNLOAD_FILE_NAME = "DiscoveryKidsHD-update.apk"
