 ...
 *   En onPause(), el bloque `isInCommercialBlock` asignaba commercialPausedMs con
 *   videoView.currentPosition en el momento en que Android ya había pausado el VideoView,
 *   devolviendo 0 o un valor incorrecto — el mismo bug clásico resuelto para programas
 *   en BUG FIX 1998.2.0.1 mediante el positionTrackerRunnable.
 *   Como resultado, al volver de segundo plano durante un bloque comercial (enseguida3/4,
 *   comercial o ya_volvemos), videoView.seekTo(0) hacía que el video comenzara desde el
 *   principio del clip actual, rompiendo la secuencia del corte publicitario.
 *
 * Fix aplicado (2001.2.5.2):
 *   1. positionTrackerRunnable ahora también actualiza commercialPausedMs cada 16 ms
 *      cuando isInCommercialBlock es true (misma lógica que pausedPositionMs para programas).
 *   2. playCommercial() llama startPositionTracker() (en lugar de stopPositionTracker())
 *      para mantener el tracker activo durante toda la secuencia comercial.
 *   3. onPause() elimina la lectura directa de videoView.currentPosition para el bloque
 *      comercial; usa el valor ya actualizado por el tracker.
 *   4. onResume() llama startPositionTracker() después de reanudar el bloque comercial
 *      para cubrir posibles nuevos backgrounds consecutivos.
 *
 * Beta 2001.2.5.1 — BUG FIX: posición incorrecta al reanudar después de un comercial.
 *
 * Causa raíz identificada:
 *   1. saveChannelState() guardaba videoView.currentPosition cuando estaba en un bloque
 *      comercial, que es la posición del comercial activo — no la posición del programa
 *      donde debía retomarse. Al reanudar, el programa arrancaba desde ese punto incorrecto.
 *   2. resumeSavedState() para el caso "commercial" llamaba beginProgramSegment con
 *      isFirstPlay=false pero breakQueue vacío — el programa reanudaba sin ningún break
 *      programado, o en versiones anteriores con isFirstPlay=true recalculaba todos los
 *      breaks e insertaba uno ya consumido, generando un corte comercial fantasma.
 *
 * Fix aplicado:
 *   1. saveChannelState() ahora guarda commercialResumeMs (posición del programa
 *      post-comercial) en vez de currentPosition cuando isInCommercialBlock es true.
 *   2. breakQueue se serializa como String ("ms1,ms2,ms3") y se persiste en SharedPreferences
 *      con la clave PREF_BREAK_QUEUE.
 *   3. resumeSavedState() deserializa y restaura breakQueue antes de llamar
 *      beginProgramSegment, garantizando que los breaks pendientes correctos estén
 *      disponibles sin recalcular desde cero.
 *
...
class LiveDiscoveryKids : AppCompatActivity() {
    ...
    private val positionTrackerRunnable = object : Runnable {
        override fun run() {
            ...
            // BUG FIX 2001.2.5.2: también actualizar commercialPausedMs continuamente mientras
            // dura el bloque comercial. Sin esto, onPause() asignaba videoView.currentPosition
            // en el momento crítico en que Android ya pausó el VideoView, devolviendo 0 o un
            // valor incorrecto — el mismo bug clásico que el tracker resolvió para programas.
            // El tracker ya corre durante el bloque comercial porque startPositionTracker() se
            // llama en playCommercial() → este bloque cubre ese caso sin costo adicional.
            if (isInCommercialBlock && videoView.isPlaying) {
    ...
    override fun onPause() {
        ...
        when {
            ...
            isInCommercialBlock -> {
                ...
                //
                // BUG FIX 2001.2.5.2: commercialPausedMs ya viene actualizado cada 16 ms
                // por positionTrackerRunnable, igual que pausedPositionMs para programas.
                // Se elimina la lectura de videoView.currentPosition aquí porque Android
                // pausa el VideoView ANTES de llamar onPause(), devolviendo 0 o un valor
                // incorrecto en ese momento crítico.
                stopPositionTracker()
                videoView.pause()
    ...
    override fun onResume() {
        ...
        when {
            ...
            isInCommercialBlock -> {
                ...
                // BUG FIX 2001.2.5.2: reiniciar el tracker para que siga actualizando
                // commercialPausedMs en caso de que el usuario vuelva a segundo plano.
    ...
    */
    ...
    private fun saveChannelState() {
        // BUG FIX 2001.2.5.1:
        // Posición a guardar según el contexto:
        //   - Programa activo → posición del tracker (más precisa que currentPosition en onPause)
        //   - Bloque comercial → commercialResumeMs (posición del programa donde debe retomar
        //     cuando termine el bloque; no la posición del comercial en sí)
        //   - Cualquier otro ítem → currentPosition del VideoView
        val posToSave = when {
            isInProgramSegment  -> pausedPositionMs
            isInCommercialBlock -> commercialResumeMs   // FIX: guardar punto de retoma, no pos del comercial
            else                -> videoView.currentPosition.coerceAtLeast(0)
        }

        // Serializar breakQueue como "ms1,ms2,ms3" para persistirla.
        // Esto permite que al reanudar después de un comercial se conozcan los
        // breaks pendientes y no se recalculen desde cero.
        ...
        Log.d(TAG, "Estado guardado: type=$currentItemType pos=${posToSave}ms playlist=$playlistIndex breaks=$breakQueueStr")
    }
    ...
    private fun showResumeDialog(prefs: android.content.SharedPreferences) {
        ...
        val whereStr = when (itemType) {
            ...
            "talla"      -> getString(R.string.resume_where_enseguida)
