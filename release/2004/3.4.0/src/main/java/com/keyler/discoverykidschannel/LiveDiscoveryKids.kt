 ...
 * Release 2004.3.4.1
 *
 * Cambios respecto a 3.4.0:
 *   - BUG FIX: Botones Prev / Next se congelaban tras la Enseguida. El enfoque
 *     anterior (iniciar el bloque desde PlayItem.Enseguida) encadenaba dos llamadas
 *     a playUriWithTransition() — una para la enseguida y otra para el bumper —
 *     dentro del mismo ViewPropertyAnimator (instancia única de videoView.animate()).
 *     La segunda llamada cancelaba la animación de la primera antes de que su
 *     withEndAction corriera, por lo que onPrepared del bumper nunca se configuraba
 *     y el canal se quedaba congelado en la enseguida.
 *     goToAdjacentProgram() ahora salta directo al programa destino (igual que un
 *     cambio de canal). El bloque Enseguida → StandaloneCommercial → Bumper ocurre
 *     naturalmente cuando ese programa termina por su propio onCompletionListener.
 *
 * Cambios respecto a 3.3.2:
 *   - BUG FIX: ScreenBug dejaba de aparecer / reiniciaba su counter de 20 s al volver
 *     de segundo plano. scheduleSegmentLogic() ahora calcula elapsed =
 *     startOffsetMs - currentSegmentStartMs y ajusta bugShowDelay y hideAt
 *     descontando el tiempo ya transcurrido. Si elapsed >= BUG_SHOW_DELAY el
 *     screenbug aparece inmediatamente con setBugAlpha(1f) sin animación.
 *     Nueva variable currentSegmentStartMs; se actualiza en cada llamada a
 *     scheduleSegmentLogic().
 *
 *   - BUG FIX: FadeIn redundante del videoView al volver de segundo plano.
 *     beginProgramSegment() ahora llama videoView.animate().cancel() y fuerza
 *     alpha = 0f antes de setVideoURI() para garantizar estado limpio
 *     independientemente de animaciones previas interrumpidas.
 *
 *   - FEATURE: Botones Prev / Next navegan directamente al programa anterior/siguiente
 *     sin pasar por Enseguida → StandaloneCommercial → Bumper del bloque destino.
 *     goToAdjacentProgram() fija playlistIndex en el PlayItem.Program correcto y llama
 *     playProgram() directamente; el bloque de transición ocurre al terminar el programa.
 *     (3.4.1: corregido bug ViewPropertyAnimator — ver arriba.)
 *
 * Cambios respecto a 3.3.1:
 *   - onPause()/onResume() reescritos con estrategia diferenciada por tipo de ítem.
 *     PROGRAMA: beginProgramSegment() con seekTo en onPrepared al volver.
 *     NO-PROGRAMA: advance() reinicia el ítem desde el principio.
 *     COMERCIAL EN CURSO: retoma el programa en commercialResumeMs.
 *     Se eliminaron wasPlayingBeforePause, lastVideoPositionMs y el alias
 *     pausedPositionMs/lastVideoPositionMs. Nueva variable pausedByLifecycle.
 *     Diagnóstico: cancelAllTasks() en onPause() destruía los listeners de
 *     playUriWithTransition; seekTo() directo en onResume() fallaba silenciosamente
 *     si Android liberó el surface en segundo plano.
 *
 * Cambios respecto a 3.2.0:
 *   - Todos los FadeOut de cambio de video unificados a 500 ms.
 *     TRANSITION_FADE_OUT_MS = 500L. Las constantes diferenciadas por clip
 *     (ENSEGUIDA_FADE_OUT_MS, BUMPER_FADE_OUT_MS, YA_REGRESA_FADE_OUT_MS,
 *     CONTINUAMOS_FADE_OUT_MS) eliminadas; todos los callers usan el valor por defecto.
 *   - bumper2.mp4 reemplazado por material original de la Era 2004.
 *   - enseguida1.mp4 reemplazado; el programa que anunciaba dejó de transmitirse en 2004.
 *   - ya_regresa4.mp4 y continuamos4.mp4 reemplazados; el inicio de ya_regresa4
 *     estaba mezclado con audio/imagen del programa.
 *
 *
 *
 * Estrategia de pausa (Beta 3.4.0.40) — DIFERENCIADA POR TIPO DE ÍTEM:
 *   PROGRAMA  → beginProgramSegment() con seekTo en onPrepared al volver.
 *   NO-PROGR. → advance() reinicia el ítem desde el principio.
 *   COMERCIAL → retoma el programa en commercialResumeMs (salta el comercial).
 *
