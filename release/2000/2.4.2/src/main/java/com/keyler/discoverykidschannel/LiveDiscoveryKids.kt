...
/**
 * Release 2.4.2 – basado en el año 2000
 *
 * Bug fixes / cambios respecto a 2.4.1:
 *   - BUG FIX: Al ejecutarse el FadeOut del VideoView antes del bloque comercial,
 *     el alpha quedaba en 0f al terminar la animación, haciendo que los videos del
 *     bloque comercial (enseguida pre-comercial, comercial y ya_volvemos) se
 *     reprodujeran de forma invisible.
 *     Se agregó videoView.alpha = 1f (sin animación) al inicio del withEndAction
 *     del FadeOut en playCommercial(), asegurando que el bloque comercial completo
 *     sea visible desde el primer frame.
 *
 * [2.4.1] Bug fixes / cambios respecto a 2.4.0:
 *   - positionTrackerRunnable: ahora guarda la posición en tiempo real (cada 16 ms,
 *     ~60 fps) en lugar de cada 500 ms, eliminando el retraso de ~1 segundo al
 *     volver de segundo plano.
 *   - FadeOut del VideoView al comenzar el bloque comercial (playCommercial).
 *   - FadeIn del VideoView al reanudar el programa tras el comercial (beginProgramSegment).
 *
 * [2.4.0] Cambios respecto a 2000.2.3.1:
...
class LiveDiscoveryKids : AppCompatActivity() {
    ...
    private val positionTrackerRunnable = object : Runnable {
        override fun run() {
            ...
            // BUG FIX 2.4.1: se reprograma a sí mismo cada 16 ms (~60 fps / tiempo real)
    ...
    */
    private fun beginProgramSegment(uri: Uri, startOffsetMs: Int, isFirstPlay: Boolean) {
        ...
        videoView.setOnPreparedListener { mp ->
            ...
            // Nueva funcionalidad 2.4.1: FadeIn del VideoView al iniciar el programa,
            // tanto en el arranque inicial como al regresar del bloque comercial.
            videoView.alpha = 0f
            videoView.start()
            videoView.animate()
                .alpha(1f)
    ...
    private fun playCommercial(resumeProgramAtMs: Int) {
        ...
        // BUG FIX / Nueva funcionalidad 2.4.1:
        ...
        videoView.animate()
            ...
            .setDuration(500L)
            .withEndAction {
                // BUG FIX 2.4.2: Al terminar el FadeOut el alpha queda en 0f.
                // Se restablece a 1f SIN animación para que los videos del bloque
                // comercial (enseguida pre-comercial, comercial y ya_volvemos)
                // sean visibles desde el primer frame.
                videoView.alpha = 1f
    ...
    private fun startBgMusic() {
        if (bgPlayer == null) {
            ...
            bgPlayer = MediaPlayer.create(this, R.raw.bg_music)?.apply {
                ...
                setVolume(0.05f, 0.05f)  // volumen al 2% para no tapar el audio del video
